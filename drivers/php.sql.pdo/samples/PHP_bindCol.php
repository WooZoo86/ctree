<?php
   ...
   print("<html>\n");
   print("<head>\n");
   print("\t<title>PHP Bind Column</title>\n");
   print("</head>\n");
   print("<body>\n");
   ...
   function Delete_Records ($ses) {
      ...
      $qry = $ses->exec("DELETE FROM table3");
      if (!$qry)
   ...
   function Add_Records ($ses) {
	  ...
	  $qry_cmd = 'INSERT INTO table3 VALUES(:fld1,:fld2,:fld3)';
   ...
   function Display_Records ($ses) {
      ...
      $qry_cmd = 'SELECT fld1, fld2, fld3 FROM table3';
	...
	$qry->bindColumn(1, $fld1);
	$qry->bindColumn(2, $fld2);

	/* Bind by column name */
	$qry->bindColumn('fld3', $fld3);
	
    while ($row = $qry->fetch(PDO::FETCH_BOUND))
		print "\t\t\t<TD>" . $fld1 . "</TD><TD>" . $fld2 . "</TD><TD>" . $fld3 . "</TD>";
